 ...
 *                       USART4 RX: PC11, AF4, HB1, 115200-8N1
 *                       中断接收 + 状态机解析
 *******************************************************************************/
 ...
 * @brief   初始化 USART4 RX 中断接收 (PC11, AF4)
 */
void bsp_tof_tfmini_init(void)
{
    ...
    GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF4);
 ...
 * @brief   USART4 RX 中断: 接收 6 字节帧, CRC 校验后更新全局数据
 *
 * 状态机:
 *   非帧头 → 跳过等待 0xBB
 *   帧头   → 从 0 开始收 6 字节
