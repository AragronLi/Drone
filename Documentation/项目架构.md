...
ATK 十轴IMU ──USART→ PB10/11│ USART3               │
下视激光 ──USART→ PC10/11│ USART6               │
前视激光 ──USART→ PC12/PD2│ USART7               │
ATK 光流    ──SPI──→ PB3-5│  SPI3                    │
GPS         ──USART→ PA13/14│ USART4            │
                          │                          │
数传        ──USART→ PE8/7│ USART8      │
空闲/预留  ──USART→ PF5/PE0│ USART5      │
Debug       ──USART→ PB6/7│ USART1               │
                          │                          │
...
| PC11 | USART6_RX | 下视激光 | J3 | 115200 |
| PD2 | USART7_RX | 前视激光 | J4 | 115200 |
| PB3 | SPI3_SCK | ATK 光流 SCK | J5 | 2MHz, 33Ω串阻R172 |
| PB4 | SPI3_MISO | ATK 光流 MISO | J5 | 33Ω串阻R171 |
| PB5 | SPI3_MOSI | ATK 光流 MOSI | J5 | 33Ω串阻R170 |
| PC5 | GPIO | ATK 光流 CS | J5 | — |
| PA4-7 | SPI1 | 预留扩展 | HDR-M_2.54 | 备用SPI接口 |
| PB8 | SWCLK | SWD烧录 | SWD | — |
| PB9 | SWDIO | SWD烧录 | SWD | — |
| PA13 | USART4 | GPS | J6 | 115200 |
| PA14 | USART4 | GPS | J6 | 115200 |
...
| PA13 | USART4 | GPS | J6 | V5F |
| PA14 | USART4 | GPS | J6 | V5F |
| PE7 | USART8 | 数传 | J7 | V5F |
| PE8 | USART8 | 数传 | J7 | V5F |
| PF5 | USART5 | 空闲/预留 | J8 | V5F |
| PE0 | USART5 | 空闲/预留 | J8 | V5F |
...
| PB8 | SWCLK / I2C1_SCL | SWD烧录（I2C1未使用） | SWD接口 |
| PB9 | SWDIO / I2C1_SDA | SWD烧录（I2C1未使用） | SWD接口 |
| PA13 | USART4 | GPS | GPS接口 |
| PA14 | USART4 | GPS | GPS接口 |
...
| SPI | 引脚 | 设备 | 备注 |
|-----|------|------|------|
| SPI1 | PA4/5/6/7 | 预留扩展 | HDR-M_2.54接口，备用 |
| SPI2 | PB12/13/14/15 | SD 卡 | XKTF-015-G卡座 |
| SPI3 | PB3/4/5 + PC5(CS) | ATK 光流 PMW3901 | 2MHz, GH1.25-6P, 33Ω串阻 |

### 2.5 I2C 分配

| I2C | SCL | SDA | 设备 | 速率 | 备注 |
|-----|-----|-----|------|------|------|
| USART4 | GPS | PC6/PC7 | 38400（默认），NMEA |
...
| 通道 | 引脚 | 检测项 | 电路 | 计算公式 |
|------|------|--------|------|---------|
| ADC1_IN1 | PA1 | 电池电压 | R179(10kΩ)+R180(2kΩ), 1:6分压, C105(100nF)滤波 | V_bat = ADC × (3.3/4096) × 6.0 |
| (ADC1_IN2) | PA2 | 预留 | 未使用 | — |

**电池电压检测说明**：
- 4S满电 16.8V → PA1读取 2.8V (在ADC量程内)
- 4S低电 14.0V → PA1读取 2.33V
- ADC参考电压：VDD33A (3.3V模拟电源)
...
| J3 | 下视激光 | GH1.25-4P | 4 | GND / 5V / TX / — |
| J4 | 前视激光 | GH1.25-4P | 4 | GND / 5V / TX / — |
| J5 | ATK 光流 | GH1.25-6P | 6 | GND / 3.3V / SCK / MISO / MOSI / CS |
| J6 | GPS | GH1.25-6P | 6 | GND / 5V / TX / RX / SDA(PA13) / SCL(PA14) |
| J7 | 数传 | XH2.54-4P | 4 | GND / 3.3V / TX / RX |
| J8 | 空闲/预留 | XH2.54-4P | 4 | GND / 3.3V / TX / RX |
| J9 | Debug 串口 | HDR-M_2.54-4P | 4 | GND / 3.3V / TX / — |
| J10 | ESC ×4 | SH1.0-6P | 6 | GND / S1 / S2 / S3 / S4 / — |
| J11 | 蜂鸣器 | XH2.54-2P | 2 | GND / SIG |
| J12 | SWD 烧录 | HDR-M_2.54-4P | 4 | GND / 3.3V / SWDIO(PB9) / SWCLK(PB8) |
| J13 | SD 卡 | XKTF-015-G | — | 板载贴片 |
| J14 | GPS 串口 | HDR-M_2.54-4P | 4 | GND / 3.3V / SDA(PA13) / SCL(PA14) |
...
```
VBAT 电池输入 (14.8~16.8V, 4S LiPo)
  │
  ├──→ SY8088AAC (U12) 开关降压 → 5V / 2A
  │     │
  │     ├── J1  → IA6B 接收机 (5V, ~100mA)
  │     ├── J3  → 下视激光 (5V, ~120mA)
  │     ├── J4  → 前视激光 (5V, ~120mA)
  │     ├── J6  → GPS (5V, ~50mA)
  │     └── J11 → 蜂鸣器 (5V, ~80mA, 通过S8050驱动)
  │
  ├──→ SY8088AAC (U14) 开关降压 → 5V
  │     │
  │     └──→ TPS54202DDCR (U15) 降压 → 3.3V (VDD33, 数字电源)
  │           │
  │           ├── CH32H417 数字IO (3.3V, ~150mA)
  ...
  │           └── J13 → SD 卡 (3.3V, ~100mA)
  │
  ├──→ MPZ1608S601ATA00 (L6) 电感滤波 → VDD33A (模拟3.3V)
  │     └── CH32H417 模拟电源 (ADC, DAC, 比较器)
  │
  ├──→ 内部LDO → 1V2A (核心电压)
  │
  └──→ 电阻分压 (R179 10kΩ + R180 2kΩ) → PA1 (ADC1_IN1, 电池电压检测)
       分压比 1:6, 满电16.8V → 2.8V
```

**关键特性**：
- 双路5V电源：U12供外设，U14供MCU电源链
- 数字/模拟电源分离：VDD33 (数字) 与 VDD33A (模拟) 独立供电
- 电感：L1 4.7μH (U12), L6 MPZ1608S601ATA00 (VDD33A滤波)
- 去耦电容：每个电源引脚配100nF陶瓷电容 + 4.7μF钽电容

### 4.2 功率估算

| 电压轨 | 峰值电流 | 设计余量 | 芯片型号 |
|--------|---------|---------|---------|
| 5V | ~470mA | **2A** | SY8088AAC (U12/U14) |
| 3.3V (VDD33) | ~770mA | **2A** | TPS54202DDCR (U15) |
| 3.3V (VDD33A) | ~50mA | 滤波 | MPZ1608S601ATA00 电感 |
| 1.2V (核心) | ~200mA | 内部LDO | CH32H417内部 |
...
| vTaskTelemetry | 10Hz | 中 | MAVLink 遥测发送 (数传 + 空闲/预留) |
| vTaskCompass | 50Hz | 高 | 罗盘旧 I2C3 方案已作废读取 + 航向计算（规划） |
...
float height_m = tof_altitude;                 // 下视激光高度 (USART4)
...
| 项目 | GPS (USART4) | 罗盘 (I2C3) |
|------|-------------|------------|
| 接口 | USART4 (PC6/PC7) | — |
| 波特率/速率 | 115200, 8N1 | — |
| 协议 | NMEA ($GNGGA, $GNRMC) | I2C 寄存器读写 |
| 帧率 | 10Hz | 50Hz |
| 芯片 | u-blox M8N / ATGM336H | HMC5883L / 罗盘旧方案 |

**罗盘 (I2C) 读取流程:**
```c
// 罗盘旧方案 典型初始化 + 读取 (I2C3, 地址 0x0D)
...
│       ├── sensor_tof_down.c/.h      # 下视激光
│       ├── sensor_flow.c/.h          # ATK 光流 SPI
│       ├── bsp_esc_pwm.c/.h          # TIM1 4路 50Hz PWM
│       ├── bsp_adc.c/.h              # 电池 ADC
│       ├── sensor_compass.c/.h       # 罗盘旧 I2C3 方案已作废 (罗盘旧方案；当前需接入工程编译和Hardware调用)
│       └── bsp_ipc.c/.h              # IPC 通信
│
├── V5F/                              # 大核工程 (FreeRTOS)
│   ├── Big_Core.wvproj
...
│       ├── system_ch32h417.c/.h
│       ├── FreeRTOSConfig.h
│       ├── sensor_tof_forward.c/.h   # 前视激光
│       ├── sensor_gps.c/.h           # GPS NMEA 解析
│       ├── comm_lora.c/.h            # 数传 MAVLink
│       ├── comm_wifi.c/.h            # 空闲/预留 MAVLink
...
| 编号 | 元件 | 型号/值 | 封装 | 数量 |
|------|------|---------|------|------|
| U1 | 飞控 MCU | CH32H417QEU6 | QFN128 | 1 |
| U12/U14 | 开关降压 | SY8088AAC | SOT23-6 | 2 |
| U15 | 3.3V降压 | TPS54202DDCR | SOT23-6 | 1 |
...
| 光流 | ATK 4m SPI | 1 |
| GPS | 串口 GPS + I2C 罗盘 (M8N+罗盘旧方案) | 1 |
| 数传 | 数传 串口 | 1 |
| 空闲/预留 | 空闲/预留 | 1 |
| 蜂鸣器 | 有源 5V | 1 |
| OLED | 0.96 寸 OLED旧方案 (I2C, 0x3C) | 1 |
...
| P2 | ATK IMU + IBUS + TOF + GPS 驱动 | 串口输出各传感器数据 |
